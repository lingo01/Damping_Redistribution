...
parallelAdmittance = 5e4j;

%% Dimension scaling

dynaDevNum = linspace(1, 960/length(dynaDevPos), 25)' * ones( 1, length(dynaDevPos) );
...
for rr = 1 : size(dynaDevNum, 1)
    ...
    for ii = 1:length(dynaDevPos)
        ...
        for jj = 1:dynaDevNum(rr, ii)
            ...
            if strcmp(type{dynaDevPos(ii)}, "Gen")
                ...
                Gen{dynaDevPos(ii)}.M  = Gen{dynaDevPos(ii)}.M    / dynaDevNum(rr,ii);
    ...
    if rr == 1
        figure(144); subplot(3,2,5); hold on; box on; grid on;
        plot(eig(mat.T_uncouple), 'o', 'LineWidth', 1.2);  xlim([-1 0]);
        plot(eig(mat.T), '*', 'LineWidth', 1.2);  xlim([-1 0]);
        xlabel('Re(\lambda)', 'FontName', 'Times New Roman');
        ylabel('Im(\lambda)', 'FontName', 'Times New Roman');
        title('(d)', 'FontName', 'Times New Roman')
        legend('eig(T_n)', 'eig(T)', 'FontName', 'Times New Roman', 'location', 'northwest')
    elseif rr == size(dynaDevNum, 1)
        figure(144); subplot(3,2,6); hold on; box on;  grid on;
        plot(eig(mat.T_uncouple), 'o', 'LineWidth', 1.2);  xlim([-1 0]);
        plot(eig(mat.T), '*', 'LineWidth', 1.2);  xlim([-1 0]);
        xlabel('Re(\lambda)', 'FontName', 'Times New Roman');
        ylabel('Im(\lambda)', 'FontName', 'Times New Roman');
